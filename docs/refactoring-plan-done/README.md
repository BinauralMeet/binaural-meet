...
**いつ読むか**: `refactor/architecture-cleanup` ロードマップの各フェーズで何を判断したか調べる / 当時なぜその設計を選んだか(採らなかった案も含め)確認する
...
## 2. Participant 継承構造の修正 {#participant-hierarchy}
...
## 3. Content の型別データ構造化 {#content-type-structure}
...
## 4. 実施手順（推奨順序） {#implementation-order}
